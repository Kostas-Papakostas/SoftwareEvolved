...
public class GeneralTableGraphicComputation extends TableComputation{
    ...
    public final JvTable makeGeneralGraphicTable(MyTableModel zoomModel_p, int rowHeight, int columnWidth, int wholeCol) {
        ...
        int start = -1;
        int end = -1;
        if (ProjectConfigurator.getGlobalDataKeeper().getPhaseCollectors() != null && wholeCol != -1 && wholeCol != 0) {
            start = ProjectConfigurator.getGlobalDataKeeper().getPhaseCollectors().get(0).getPhases().get(wholeCol - 1).getStartPos();
            end = ProjectConfigurator.getGlobalDataKeeper().getPhaseCollectors().get(0).getPhases().get(wholeCol - 1).getEndPos();
        }
